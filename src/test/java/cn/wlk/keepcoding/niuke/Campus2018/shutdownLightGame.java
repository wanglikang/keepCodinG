 ...
 * 给个解释（猜测）：因为每次操作都会将某灯的右边所有灯都翻转，那么在最后一次Alice操作的时候，
 * 最右边那个灯的状态和最开始的状态必定正好相同（经过了两人偶数次操作）；
 * 即若最开始是亮的，最后一次必定是还是亮的，翻转一下就赢了，否则，已经被Bob翻转过了，Bob已经赢过了。
 * （因为两个人都是聪明的，那就会从左往右找亮的灯进行翻转）
 */
